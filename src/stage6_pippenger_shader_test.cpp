// Stage 6 validation: GPU Pippenger MSM, cross-checked against the
// already-validated CPU Pippenger implementation (src/msm.cpp, Stage 3).
//
// Design: the host loops over windows (same structure as msm_pippenger()
// on the CPU). For each window, a GPU dispatch fills all buckets in
// parallel using a conflict-free scheme - one thread per BUCKET (not per
// point), each thread scanning every point and accumulating only the ones
// matching its own bucket index, so no two threads ever write the same
// memory and no atomics are needed. The much cheaper "combine buckets
// into a window total" reduction (only num_buckets points, vs. num_points
// inputs) runs back on the CPU host using the Stage 2 point_add.
//
// blst is used only to source the G1 generator (an already-trusted
// constant, per Stage 3.2's pattern) - random test points are then
// derived via our own validated point_scalar_mul, not blst_p1_mult.
...
// The G1 generator's coordinates, sourced from blst (same pattern as
// Stage 3.2's stage3b_pippenger_test.cpp).
...
// Vulkan context shared across all test sizes.
struct GpuContext {
    VkInstance instance;
...
// Runs GPU Pippenger for one (points, scalars) test case and returns the
// final result, combining bucket sums on the CPU between GPU dispatches.
PointJacobian run_gpu_pippenger(GpuContext& ctx, const std::vector<PointJacobian>& points,
    ...
    pc.window_bits = static_cast<uint32_t>(window_bits);
    pc.num_points = n;
    pc.num_buckets = ctx.numBuckets;

    int num_windows = (kScalarBits + window_bits - 1) / window_bits;
    ...
    for (int w = num_windows - 1; w >= 0; w--) {
        ...
        // Running-sum combination (CPU, Stage 2) - identical structure to
        // msm_pippenger()'s inner reduction in src/msm.cpp.
...
int main() {
    try {
        ...
        appInfo.pApplicationName = "vkmsm-stage6";
        ...
        std::printf("window_bits=%d, num_buckets=%u\n\n", window_bits, ctx.numBuckets);
        ...
        for (int n : {0, 1, 2, 3, 37, 256, 1024}) {
            ...
            PointJacobian actual = run_gpu_pippenger(ctx, points, scalars, window_bits);
